...
function Report(dir_name){
	this.dir = dir_name;	
}
Report.prototype.Create_rep = function(data){
	var date_creation = new Date();
	var file_name='/report_'+String(date_creation.getDate())+'_'+(date_creation.getMonth()+1<10?'0':'')+String(date_creation.getMonth()+1)
	+'_'+String(date_creation.getFullYear())+'('+String(date_creation.getHours())+'h_'+String(date_creation.getMinutes())+'m_'+String(date_creation.getSeconds())+'s).html';
    this.file_path=this.dir+file_name;
      //fs.openSync(report_path.toString(),'a'); // creates file
       
    fileSystem.openSync("D:\\TRS\\jtrs\\report\\testing.html",'a');
    //fileSystem.writeFileSync("D:\\TRS\\jtrs\\report\\testing.html", Report.prototype.CreateHTMLTemplate().toString());
    Report.prototype.ParseRepContent(data);
    

}
Report.prototype.ParseRepContent = function(data){
var report_tags = "<html> <body> </body> </html>";
var par = report_tags.toString();
par = par.split(" ");
var index = par.indexOf("<body>");
par.splice(index+1,0,"<div>");
par.splice(index+2,0,"</div>");
var report_data="";
report_data+="<li>Suite name: "+data.split('|')[0]+"</li>";
report_data+="<li>Path to test: "+data.split('|')[1]+"</li>";
report_data+="<li>Executed test: "+data.split('|')[2]+"</li>";
report_data+="<li>Execution time: "+"Test speed"+"</li>";
index = par.indexOf("<div>");
par.splice(index+1,0,report_data.toString());
par = par.toString().replace(/,/g,"");
fileSystem.writeFileSync("D:\\TRS\\jtrs\\report\\testing.html", par.toString());
}
