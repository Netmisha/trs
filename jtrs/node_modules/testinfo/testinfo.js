...
var rootNode={"id":"0","path":"","disable":"", "name":{}, "tests":[], "children":[]};
function ParseSuite(suite, node) {
    node.id=suite.suiteId;
    node.path=suite.path;
    node.name=suite.suite.$.name;
    node.disable=suite.suite.disable;
    for(var i=0; i<Object.keys(suite.suite.test).length; i++) {
        node.tests[i]={'id':suite.suiteId+"-0-"+String(i+1), 'name':suite.suite.test[i].$.name, 'path':node.path, "disable":suite.suite.test[i].disable};
    }
    for(var i=0; i<Object.keys(suite.children).length; i++) {
        node.children[i]={"id":"0","path":"","disable":"", "name":{}, "tests":[], "children":[]};
        ...
        if (stat && stat.isDirectory()) {
            ...
            if(file.split('.').pop()=='xml') {
                parser.parseString(fileSystem.readFileSync(file).toString(),function (err, result) {
                    ...
                    rootSuite.path=file.substr(file.indexOf('jtrs')+5, file.length);
                });
            }
        }
    });
    childDir.forEach(function(file){
        rootSuite.children[index]={"suiteId":suiteId+"-"+String(index+1), "path":"", "suite":{}, "children":[]};
...
function FindTests(dir, suiteId) {
    ...
    if(file!="") {
        var parser = new xml2js.Parser();
        var testsDir;
        parser.parseString(fileSystem.readFileSync(dir+file).toString(),function (err, result) {
